...
export default function EditFlashcardPage({ params }: EditFlashcardPageProps) { // Use the explicit props interface
  ...
  useEffect(() => {
    if (cardID !== 'new' && !loading && loggedInSubject && loggedInSubject.toLowerCase() === subject.toLowerCase()) {
      const fetchFlashcard = async () => {
        ...
        if (error || !data) {
          ...
          setCardToEdit([{ id: data.id, front_text: data.front_text, back_text: data.back_text }]);
        }
        setPageLoading(false);
      };
      fetchFlashcard();
    } else {
      router.push(`/dashboard/${subject}/flashcards/new`); // Redirect to the proper new page
    }
  ...
  const handleSaveFlashcard = async () => {
    setIsSaving(true);
    if (cardToEdit.length === 0 || !cardToEdit[0].front_text || !cardToEdit[0].back_text || !flashcardGrade) {
  ...
  if (loading || pageLoading || !loggedInSubject || loggedInSubject.toLowerCase() !== subject.toLowerCase()) {
    return (
  ...
  return (
    <div className="p-8 min-h-screen flex flex-col">
      <div className="flex items-center mb-6">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-arrow-left text-white"><path d="m12 19-7-7 7-7"/><path d="M19 12H5"/></svg>
        </button>
      ...
      <div className="flex justify-end space-x-4 mt-auto">
        ...
        <button
          ...
          disabled={isSaving || cardToEdit.length === 0 || !cardToEdit[0].front_text || !cardToEdit[0].back_text || !flashcardGrade}
